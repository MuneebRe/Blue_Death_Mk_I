...
void Camera::acquire()
{
	...
	{								//If it's a camera, use the camera function
		...
		screen_grab((unsigned char*)(rgb.pdata),
...
void Camera::processing()
{
	...
	case 1:
		...
		invert(a, b);			//Invert - make it temporary later
		copy(b, a);
	...
	case 6:				//Centroid from Hue test
		hue_filter(0, 6, 0.60, 0.99, 200, 255);			//Red Filter
		break;
	case 7:
...
int Camera::track_object(double& ic, double& jc)           //Code from lecture
{
